...
def update_config(config_path: Path, waypoint_name: str, pose_x: float, pose_y: float,
    ...
    # Allow bootstrapping a new config whose robot coordinates are not filled yet.
    if len(robot_points) < len(rmf_points):
        robot_points.extend([[0.0, 0.0] for _ in range(len(rmf_points) - len(robot_points))])
