...
class RobotAPI:
    def __init__(self, prefix: str, user: str, password: str, node, api_config=None):
        ...
        self._last_nav_result_status = GoalStatus.STATUS_UNKNOWN
    ...
    def navigate(self, robot_name: str, pose, map_name: str):
        ...
        self.node.get_logger().info(
            f"Sending Nav2 goal for robot [{robot_name}] "
            f"map=[{map_name}] pose=({float(pose[0]):.3f}, {float(pose[1]):.3f}, "
            f"{math.degrees(float(pose[2])):.2f} deg)"
        )
        ...
        if goal_handle is None or not goal_handle.accepted:
            self.node.get_logger().warn(
                f"Nav2 goal was rejected for robot [{robot_name}]"
            )
            with self._lock:
                self._goal_done = True
            return False
    ...
    def _result_cb(self, future):
        ...
        with self._lock:
            ...
            self._goal_pose = None
            self._goal_start_time = None
            self._last_nav_result_status = status

        self.node.get_logger().info(
            f"Nav2 result received: status={status} succeeded={succeeded}"
        )
