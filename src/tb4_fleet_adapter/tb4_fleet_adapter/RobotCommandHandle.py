...
class RobotCommandHandle(adpt.RobotCommandHandle):
    def __init__(self,
        ...
        # adapter 起動直後の初期動作:
        # 旧設定 `startup_undock_to_pre_dock` が true の場合は、
        # 「Undock する」+「pre_dock へ移動する」の両方を有効にする。
        legacy_startup_undock_to_pre_dock = bool(
            self.config.get("startup_undock_to_pre_dock", True)
        )
        self._startup_undock_on_adapter_start = bool(
            self.config.get(
                "startup_undock_on_adapter_start",
                legacy_startup_undock_to_pre_dock,
            )
        )
        self._startup_move_to_pre_dock_on_adapter_start = bool(
            self.config.get(
                "startup_move_to_pre_dock_on_adapter_start",
                legacy_startup_undock_to_pre_dock,
            )
        )
        ...
        if (
            self._startup_undock_on_adapter_start
            or self._startup_move_to_pre_dock_on_adapter_start
        ):
    ...
    def _start_startup_sequence(self):
        ...
        def _run_startup_sequence():
            ...
            try:
                ...
                if self._startup_undock_on_adapter_start and docked_state is not False:
                    self.node.get_logger().info(
                ...
                # 2) pre_dock 移動が無効なら、undock 完了時点で終了
                if not self._startup_move_to_pre_dock_on_adapter_start:
                    self.node.get_logger().info(
                        f"Startup sequence completed: robot [{self.name}] "
                        "finished startup undock without pre_dock navigation"
                    )
                    return

                # 3) pre_dock を推定し、ドック向きに整列して待機
    ...
    def follow_new_path(
        ...
        if len(self.remaining_waypoints) > 1:
            with self._lock:
                current_on_waypoint = self.on_waypoint
                current_position = copy.copy(self.position)
            while len(self.remaining_waypoints) > 1:
                first_pair = self.remaining_waypoints[0]
                first_wp = first_pair[1]
                first_index = first_wp.graph_index
                first_pose = first_wp.position
                first_dist = self.dist(current_position, first_pose)
                if (
                    (current_on_waypoint is not None and first_index == current_on_waypoint)
                    or first_dist < 0.20
                ):
                    self.node.get_logger().info(
                        f"Skipping initial already-reached waypoint "
                        f"[index={first_index}, dist={first_dist:.2f} m] "
                        f"for robot [{self.name}]"
                    )
        ...
        def _follow_path():
            ...
            while (
                ...
                if self.state == RobotState.IDLE:
                    ...
                    current_waypoint_pair = self.remaining_waypoints[0]
                    self.target_waypoint = current_waypoint_pair[1]
                    self.path_index = current_waypoint_pair[0]
                    ...
                    self.node.get_logger().info(
                        f"Robot [{self.name}] attempting waypoint "
                        f"[graph_index={self.target_waypoint.graph_index}] "
                        f"rmf=({target_pose[0]:.3f}, {target_pose[1]:.3f}, "
                        f"{math.degrees(target_pose[2]):.2f} deg) "
                        f"robot=({x:.3f}, {y:.3f}, {math.degrees(theta):.2f} deg)"
                    )
                    # ------------------------ #
                    ...
                    with self._lock:
                        ...
                        if request_done and self.api.navigation_completed(self.name):
                            self.node.get_logger().info(
                                ...
                                f"waypoint")
                            if (
                                current_waypoint_pair is not None
                                and len(self.remaining_waypoints) > 0
                                and self.remaining_waypoints[0] == current_waypoint_pair
                            ):
                                self.remaining_waypoints = self.remaining_waypoints[1:]
                            self.state = RobotState.WAITING
    ...
    def dock(
        ...
        def _dock():
            ...
            recovery_attempt = 0
            dock_recovery_pause_sec = float(
                self.config.get("dock_recovery_pause_sec", 1.0)
            )

            def _recover_with_retries(reason_text):
                nonlocal recovery_attempt
                while recovery_attempt < max_recovery_attempts:
                    recovery_attempt += 1
                    self.node.get_logger().warn(
                        f"{reason_text}; stopping and retrying dock search "
                        f"({recovery_attempt}/{max_recovery_attempts})."
                    )
                    self.api.stop(self.name)
                    self.sleep_for(dock_recovery_pause_sec)
                    if _attempt_dock_visibility_recovery():
                        return True
                return False

            if initial_visibility_failed:
                if not _recover_with_retries(
                    f"Dock is not visible for robot [{self.name}]"
                ):
                    _wait_at_pre_dock_and_finish(
                        f"Dock is not visible for robot [{self.name}] after "
                        "repeated search retries; waiting at pre_dock."
                    )
                    return

            while True:
                if not self.api.start_process(self.name, self.dock_name, self.map_name):
                    if _recover_with_retries(
                ...
                if abort_reason is not None:
                    if _recover_with_retries(abort_reason):
                        continue
                    _wait_at_pre_dock_and_finish(
