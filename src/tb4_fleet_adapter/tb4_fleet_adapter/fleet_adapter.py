...
def initialize_fleet(config_yaml, nav_graph_path, node, use_sim_time, server_uri):
    def _robot_api_settings(fleet_manager_config, robot_name, robot_entry):
        api_config = copy.deepcopy(fleet_manager_config.get("api", {}))
        api_config.update(copy.deepcopy(robot_entry.get("api", {})))

        prefix = (
            api_config.get("prefix")
            or robot_entry.get("prefix")
            or fleet_manager_config.get("prefix")
            or f"/{robot_name}"
        )
        user = robot_entry.get("user", fleet_manager_config.get("user", ""))
        password = robot_entry.get(
            "password", fleet_manager_config.get("password", "")
        )
        return prefix, user, password, api_config
    ...
    fleet_manager_config = fleet_config.get("fleet_manager", {})
    robot_apis = {}
    for robot_name, robot_entry in config_yaml["robots"].items():
        prefix, user, password, api_config = _robot_api_settings(
            fleet_manager_config, robot_name, robot_entry
        )
        node.get_logger().info(
            "Creating RobotAPI for robot [%s] namespace [%s]" %
            (robot_name, prefix)
        )
        robot_api = RobotAPI(prefix, user, password, node, api_config)
        robot_apis[robot_name] = robot_api
        if robot_api.connected:
            node.get_logger().info(
                f"RobotAPI initial connection is ready for robot [{robot_name}]"
            )
        else:
            node.get_logger().warn(
                f"RobotAPI initial connection is not ready yet for robot "
                f"[{robot_name}]; continuing to wait in registration loop"
            )
